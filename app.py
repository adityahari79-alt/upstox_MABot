...
def get_option_instrument_token(strike, expiry_date, upstox_client):
    try:
        instruments = upstox_client.get_instruments('NFO')
    except Exception:
        return None
...
class UpstoxSubscriber:
    ...
    def on_connect(self):
        ...
        self.upstox_client.subscribe(st.session_state.subscribed_tokens)

    def on_ticks(self, ticks):
        for tick in ticks:
            asyncio.run_coroutine_threadsafe(process_tick(tick, self.upstox_client), st.session_state.loop)
...
async def process_tick(tick, upstox_client):
    try:
    ...
    if (last['ma10'] >= last['ma21'] and
        ...
        opt_id = get_option_instrument_token(strike, st.session_state.expiry_date, upstox_client)
        if not opt_id:
        ...
        if st.session_state.paper_mode:
            ...
            try:
                order = upstox_client.place_order(
                    opt_id,
    ...
    if st.session_state.position:
        try:
            if st.session_state.paper_mode:
                ...
                quote_response = upstox_client.get_live_feed([st.session_state.position['option_id']])
        ...
        if ltp_opt <= st.session_state.position['sl_price']:
            if st.session_state.paper_mode:
                ...
                try:
                    sell_order = upstox_client.place_order(
...
def trading_bot_page():
    ...
    if start_bot:
        ...
        try:
            upstox_client = Upstox(API_KEY, ACCESS_TOKEN)
        ...
        subscriber = UpstoxSubscriber(upstox_client)

        upstox_client.start_websocket(subscriber.on_ticks,
                                     MarketFeedType.Full,
